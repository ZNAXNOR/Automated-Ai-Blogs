...
export const r3Draft = ai.defineFlow(
  ...
  async (input) => {
    console.log(`[r3Draft] Starting draft generation for pipeline: ${input.pipelineId}`);
    const {pipelineId, outline, researchNotes} = input as any;

    if (!outline.sections || outline.sections.length === 0) {
      console.warn("[r3Draft] No sections in outline. Returning a minimal draft.");
    ...
    const sectionPromises = outline.sections.map(async (section: any) => {
      ...
      try {
        console.log(`[r3Draft] Generating content for section: \"${section.heading}\"`);
        ...
        console.error(`[r3Draft] Failed to generate section \"${section.heading}\":`, error);
    ...
    console.log(`[r3Draft] Successfully generated content for ${generatedSections.length} sections.`);
    ...
    try {
      ...
      console.error("[r3Draft] Final draft assembly failed. Creating fallback draft.", error);
...
");
    ...
    console.log(`[r3Draft] Draft generation complete. Total length: ${output.fullDraft.length} characters.`);
