...
import { r2_angle_input, r2_angle_output } from '../../schemas/flows/r2_angle.schema.js';
import { safeParseJsonFromAI } from '../../clients/aiParsing.client.js';
import { urlContextTool } from '../../tools/urlContext.tool.js';
 ...
 * @returns A promise that resolves to the summarized context or null if it fails.
 */
async function fetchUrlWithRetry(url: string, retries = 2): Promise<any> {
  for (let i = 0; i < retries; i++) {
    try {
      console.log(`[r2_angle] Fetching context for: ${url} (Attempt ${i + 1})`);
      ...
      console.warn(`[r2_angle] Summary for ${url} is too short. Retrying...`);
    } catch (err) {
      console.error(`[r2_angle] Failed to fetch ${url} on attempt ${i + 1}:`, err);
...
export const r2_angle = ai.defineFlow(
  ...
  async (input) => {
    console.log('[r2_angle] Starting flow with input:', input);
    const { pipelineId, references, ...idea } = input as any;

    if (!references || references.length === 0) {
      console.warn('[r2_angle] No references provided. Proceeding without research notes.');
    }
    ...
    console.log(`[r2_angle] Collected ${researchNotes.length} research notes.`);

    // Use an AI prompt to synthesize the research and idea into a structured outline.
    const anglePrompt = ai.prompt('Round2_AnglePrompt');
    ...
    if (!rawOutput) {
      throw new Error('[r2_angle] AI prompt returned no usable output.');
    }

    // Parse the AI response and validate it against the output schema.
    const angleResult = safeParseJsonFromAI(rawOutput);
    const finalOutput = r2_angle_output.parse({
      ...angleResult,
      pipelineId,
      researchNotes,
    });

    console.log(`[r2_angle] Generated outline with ${finalOutput.outline.sections.length} sections.`);
