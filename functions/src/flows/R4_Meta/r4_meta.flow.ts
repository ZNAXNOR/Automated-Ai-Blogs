...
import {r4MetaInput, r4MetaOutput, R4MetaOutput} from "../../schemas/flows/r4_meta.schema";
 ...
 * @return A promise that resolves to an array of term names.
 */
async function fetchWPTaxonomy(endpoint: string): Promise<string[]> {
  try {
    ...
    console.error(`[r4Meta] Error fetching WordPress taxonomy from ${endpoint}:`, error);
...
export const r4Meta = ai.defineFlow(
  ...
  async (input) => {
    ...
    console.log(`[r4Meta] Starting metadata generation for pipeline: ${pipelineId}`);
    ...
    try {
      ...
      console.log(`[r4Meta] Fetched ${tags.length} tags and ${filteredCategories.length} categories for context.`);
      ...
      console.log(`[r4Meta] Successfully generated and validated metadata for title: \"${finalOutput.title}\"`);
    } catch (error) {
      console.error("[r4Meta] Metadata generation failed. Using fallback.", error);
