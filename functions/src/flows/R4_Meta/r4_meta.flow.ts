/**
 * @file Generates metadata for the blog post, including SEO details, tags, and categories.
 * @author Omkar Dalvi
 *
 * This flow (Round 4) takes the drafted blog content and generates essential metadata
 * required for publication and search engine optimization. It performs the following steps:
 * 1. Fetches existing tags and categories from a WordPress instance to provide context to the AI.
 *    This helps in aligning the generated metadata with the site's existing taxonomy.
 ...
 * 3. Includes a fallback mechanism to generate basic metadata if the AI prompt fails.
...
import { r4_meta_input, r4_meta_output, R4MetaOutput } from '../../schemas/flows/r4_meta.schema.js';
...
import { WP_API_URL_CONFIG } from '@src/config.js';

console.log('[r4_meta] Flow module loaded');

/**
 * Fetches taxonomy terms (like tags or categories) from a WordPress REST API endpoint.
 * @param endpoint The WordPress API URL for the taxonomy.
 * @returns A promise that resolves to an array of term names.
 */
async function fetchWPTaxonomy(endpoint: string): Promise<string[]> {
  try {
    ...
    console.error(`[r4_meta] Error fetching WordPress taxonomy from ${endpoint}:`, error);
...
export const r4_meta = ai.defineFlow(
  ...
  async (input) => {
    const { pipelineId, title, fullDraft, topic, tone } = input as any;
    console.log(`[r4_meta] Starting metadata generation for pipeline: ${pipelineId}`);
    ...
    try {
      ...
      const [tags, categories] = await Promise.all([
        fetchWPTaxonomy(`${wordpressApiUrl}/wp-json/wp/v2/tags?per_page=100&_fields=name`),
        fetchWPTaxonomy(`${wordpressApiUrl}/wp-json/wp/v2/categories?per_page=100&_fields=name`),
      ]);

      const filteredCategories = categories.filter(cat => cat.toLowerCase() !== 'uncategorized');
      console.log(`[r4_meta] Fetched ${tags.length} tags and ${filteredCategories.length} categories for context.`);
      ...
      finalOutput = r4_meta_output.parse({ ...parsedJson, pipelineId });
      console.log(`[r4_meta] Successfully generated and validated metadata for title: "${finalOutput.title}"`);

    } catch (error) {
      console.error('[r4_meta] Metadata generation failed. Using fallback.', error);
      // Fallback to basic metadata generation to ensure the pipeline continues.
      finalOutput = {
        pipelineId,
        title,
