...
import {r8PublishInput, r8PublishOutput, R8PublishInput} from "../../schemas/flows/r8_publish.schema";
 ...
 * @return A promise that resolves to the structured payload for the WP API.
 */
async function buildPostPayload(input: R8PublishInput): Promise<Record<string, any>> {
  ...
  if (meta.primaryCategory) {
    try {
      ...
      console.warn(`[r8Publish] Could not ensure category \"${meta.primaryCategory}\":`, err);
    }
  }
...
export const r8Publish = ai.defineFlow(
  ...
  async (input: R8PublishInput) => {
    ...
    console.log(`[r8Publish] Starting publish flow for pipeline: ${pipelineId}`);

    try {
      // Build the final payload for the WordPress API.
      const payload = await buildPostPayload(input);
      console.log(`[r8Publish] Creating post \"${payload.title}\" with status \"${payload.status}\".`);
      ...
      const output = r8PublishOutput.parse({
        pipelineId,
        id: wpResponse.id,
      ...
      console.log(`[r8Publish] Successfully created post. ID: ${output.id}, Link: ${output.link}`);
